...
import {
  ...
  Save, X, Plus, Eye, EyeOff, RefreshCw,
} from 'lucide-react'
...
export default function ClienteDetalhe() {
  ...
  const [emailsNotif, setEmailsNotif] = useState<string[]>([])
  const [emailNotifInput, setEmailNotifInput] = useState('')
  const [whatsappsNotif, setWhatsappsNotif] = useState<string[]>([])
  ...
  const carregar = async () => {
    ...
    setCliente(c)
    setEmailsNotif(c.emails_notificacao || [])
    setWhatsappsNotif(c.whatsapps_notificacao || [])
  ...
  const handleSalvarDados = async () => {
    if (!form.nome || !form.email_contato || !form.senha_cliente) {
      setErroDados('Nome, e-mail e senha do cliente são obrigatórios.')
      return
    }
    setSalvandoDados(true); setErroDados('')
    try {
      const res = await fetch('/api/admin/clientes', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      setCliente((prev: any) => ({ ...prev, ...data.cliente }))
      setEditando(false)
    } catch (err: any) {
  ...
  const handleSalvarNotif = async () => {
    ...
    try {
      const res = await fetch('/api/admin/clientes', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, emails_notificacao: emailsNotif, whatsapps_notificacao: whatsappsNotif }),
      })
      if (!res.ok) throw new Error('Erro ao salvar')
      setNotifSalvo(true)
      setTimeout(() => setNotifSalvo(false), 3000)
    } catch { } finally {
        ...
        <div className="max-w-6xl mx-auto p-6 space-y-6">
          ...
          {aba === 'dados' && (
              ...
              <div className="fixed inset-0 z-50 bg-black/60 flex items-start justify-center overflow-y-auto p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-6">
                  <div className="flex items-center justify-between px-6 py-4 border-b">
                    <h2 className="text-lg font-bold text-maginf-gray">Editar dados do cliente</h2>
                    <button onClick={() => setEditando(false)} className="text-gray-400 hover:text-gray-600"><X className="h-5 w-5" /></button>
                  </div>
                  <div className="p-6 space-y-4">
                    {erroDados && <p className="text-red-500 text-sm bg-red-50 border border-red-200 rounded-lg px-3 py-2">{erroDados}</p>}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="sm:col-span-2">
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Nome *</label>
                        <input value={form.nome} onChange={e => setForm((p:any) => ({...p, nome: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Razão Social</label>
                        <input value={form.razao_social} onChange={e => setForm((p:any) => ({...p, razao_social: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Nome Fantasia</label>
                        <input value={form.nome_fantasia} onChange={e => setForm((p:any) => ({...p, nome_fantasia: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">E-mail de contato *</label>
                        <input type="email" value={form.email_contato} onChange={e => setForm((p:any) => ({...p, email_contato: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Celular / WhatsApp</label>
                        <input value={form.celular} onChange={e => setForm((p:any) => ({...p, celular: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Telefone</label>
                        <input value={form.telefone} onChange={e => setForm((p:any) => ({...p, telefone: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Site</label>
                        <input value={form.site} onChange={e => setForm((p:any) => ({...p, site: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Nome do responsável</label>
                        <input value={form.responsavel_nome} onChange={e => setForm((p:any) => ({...p, responsavel_nome: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Cargo do responsável</label>
                        <input value={form.responsavel_cargo} onChange={e => setForm((p:any) => ({...p, responsavel_cargo: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">E-mail do responsável</label>
                        <input type="email" value={form.responsavel_email} onChange={e => setForm((p:any) => ({...p, responsavel_email: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">WhatsApp do responsável</label>
                        <input value={form.responsavel_telefone} onChange={e => setForm((p:any) => ({...p, responsavel_telefone: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">CEP</label>
                        <input value={form.cep} onChange={e => setForm((p:any) => ({...p, cep: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Logradouro</label>
                        <input value={form.logradouro} onChange={e => setForm((p:any) => ({...p, logradouro: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Número</label>
                        <input value={form.numero} onChange={e => setForm((p:any) => ({...p, numero: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Complemento</label>
                        <input value={form.complemento} onChange={e => setForm((p:any) => ({...p, complemento: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Bairro</label>
                        <input value={form.bairro} onChange={e => setForm((p:any) => ({...p, bairro: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Cidade</label>
                        <input value={form.cidade} onChange={e => setForm((p:any) => ({...p, cidade: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">UF</label>
                        <input value={form.estado} maxLength={2} onChange={e => setForm((p:any) => ({...p, estado: e.target.value.toUpperCase()}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Senha do portal (cliente) *</label>
                        <div className="flex gap-2">
                          <div className="relative flex-1">
                            <input type={showSenhaC ? 'text' : 'password'} value={form.senha_cliente} onChange={e => setForm((p:any) => ({...p, senha_cliente: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none pr-9" />
                            <button type="button" onClick={() => setShowSenhaC(s => !s)} className="absolute right-2 top-2.5 text-gray-400">{showSenhaC ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>
                          </div>
                          <button type="button" onClick={() => setForm((p:any) => ({...p, senha_cliente: Math.random().toString(36).slice(2,8).toUpperCase()}))} className="px-2 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg" title="Gerar senha"><RefreshCw className="h-4 w-4 text-gray-500" /></button>
                        </div>
                      </div>
                      <div className="sm:col-span-2">
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Observações internas</label>
                        <textarea value={form.observacoes} onChange={e => setForm((p:any) => ({...p, observacoes: e.target.value}))} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none resize-none" />
                      </div>
                    </div>
                  </div>
                  <div className="px-6 py-4 border-t flex justify-end gap-3">
                    <button onClick={() => setEditando(false)} className="px-4 py-2 border border-gray-300 text-gray-600 rounded-lg text-sm">Cancelar</button>
                    <button onClick={handleSalvarDados} disabled={salvandoDados} className="px-6 py-2 bg-maginf-orange hover:bg-maginf-orange-dark text-white font-bold rounded-lg text-sm flex items-center gap-2 disabled:opacity-50">
                      {salvandoDados ? <><RefreshCw className="h-4 w-4 animate-spin" />Salvando...</> : <><Save className="h-4 w-4" />Salvar alterações</>}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="flex justify-end">
              <button onClick={abrirEdicao} className="flex items-center gap-2 bg-maginf-orange hover:bg-maginf-orange-dark text-white font-semibold px-4 py-2 rounded-xl text-sm transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Notificações editáveis */}
              <div className="bg-white rounded-xl p-5 shadow-sm md:col-span-2">
                <div className="flex items-center justify-between mb-4">
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Notificações de OS</p>
                  {notifSalvo && <span className="text-xs text-green-600 font-semibold flex items-center gap-1"><CheckCircle className="h-3.5 w-3.5" />Salvo!</span>}
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {/* Emails */}
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-2">E-mails adicionais</label>
                    <div className="flex gap-2 mb-2">
                      <input type="email" value={emailNotifInput} onChange={e => setEmailNotifInput(e.target.value)}
                        onKeyDown={e => { if (e.key === 'Enter' && emailNotifInput.trim()) { e.preventDefault(); const v = emailNotifInput.trim().toLowerCase(); if (!emailsNotif.includes(v)) setEmailsNotif(p => [...p, v]); setEmailNotifInput('') }}}
                        placeholder="email@empresa.com" className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      <button onClick={() => { const v = emailNotifInput.trim().toLowerCase(); if (v && !emailsNotif.includes(v)) setEmailsNotif(p => [...p, v]); setEmailNotifInput('') }}
                        className="px-3 py-2 bg-maginf-orange text-white rounded-lg font-bold text-sm"><Plus className="h-4 w-4" /></button>
                    </div>
                    <div className="flex flex-wrap gap-1.5 min-h-[28px]">
                      {emailsNotif.map(em => (
                        <span key={em} className="flex items-center gap-1 bg-orange-50 border border-orange-200 text-orange-700 text-xs px-2 py-1 rounded-full">
                          {em}
                          <button onClick={() => setEmailsNotif(p => p.filter(x => x !== em))} className="text-orange-400 hover:text-red-500 font-bold ml-0.5">×</button>
                        </span>
                      ))}
                    </div>
                  </div>
                  {/* WhatsApps */}
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-2">WhatsApp adicionais</label>
                    <div className="flex gap-2 mb-2">
                      <input type="tel" value={whatsappNotifInput} onChange={e => setWhatsappNotifInput(e.target.value)}
                        onKeyDown={e => { if (e.key === 'Enter' && whatsappNotifInput.trim()) { e.preventDefault(); const v = whatsappNotifInput.trim(); if (!whatsappsNotif.includes(v)) setWhatsappsNotif(p => [...p, v]); setWhatsappNotifInput('') }}}
                        placeholder="(11) 00000-0000" className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maginf-orange outline-none" />
                      <button onClick={() => { const v = whatsappNotifInput.trim(); if (v && !whatsappsNotif.includes(v)) setWhatsappsNotif(p => [...p, v]); setWhatsappNotifInput('') }}
                        className="px-3 py-2 bg-maginf-orange text-white rounded-lg font-bold text-sm"><Plus className="h-4 w-4" /></button>
                    </div>
                    <div className="flex flex-wrap gap-1.5 min-h-[28px]">
                      {whatsappsNotif.map(w => (
                        <span key={w} className="flex items-center gap-1 bg-green-50 border border-green-200 text-green-700 text-xs px-2 py-1 rounded-full">
                          {w}
                          <button onClick={() => setWhatsappsNotif(p => p.filter(x => x !== w))} className="text-green-400 hover:text-red-500 font-bold ml-0.5">×</button>
                        </span>
                      ))}
                    </div>
                  </div>
                ...
                <div className="mt-4 flex justify-end">
                  <button onClick={handleSalvarNotif} disabled={salvandoNotif}
                    className="flex items-center gap-2 bg-maginf-orange hover:bg-maginf-orange-dark disabled:opacity-50 text-white font-semibold px-5 py-2 rounded-xl text-sm transition-colors">
                    {salvandoNotif ? <><RefreshCw className="h-4 w-4 animate-spin" />Salvando...</> : <><Save className="h-4 w-4" />Salvar notificações</>}
                  </button>
                </div>
              </div>
